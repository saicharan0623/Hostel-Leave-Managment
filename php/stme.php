...
// Set finalPosition in the session
$_SESSION["finalPosition"] = "Rector"; // Replace with the actual value

// Retrieve finalPosition from the session
if (isset($_SESSION["finalPosition"])) {
    $finalPosition = $_SESSION["finalPosition"];
} else {
    // Handle the case where finalPosition is not set
...
$query = "SELECT * FROM leave_applications WHERE school = 'STME'";
$stmt = $pdo->prepare($query);
$stmt->execute();
...
<body>
    <h1>Welcome to STME Dashboard</h1>
