...
$query = "SELECT * FROM leave_applications WHERE school = 'SPTM'";
$stmt = $pdo->prepare($query);
$stmt->execute();
...
<body>
   <h1>Welcome to SPTM Dashboard</h1>
    ...
    <table>
        ...
        <tbody>
            <?php
            // Loop through the STME school data and populate the table rows
            foreach ($stmeData as $row) {
                ...
                echo '<td class="student-details">' . $row['email'] . '</td>';
